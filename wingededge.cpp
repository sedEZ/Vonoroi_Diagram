...
void WingedEdge::constructTwoPointsVoronoi()
{
    ...
    if(fabs(y_1 - y_2)< 1e-8){//Vertical median line
        ...
        //qDebug()<<"x[0] = "<<this->x[0]<<"; x[1] = "<<this->x[1]<<"; x[2] = "<<this->x[2];
        //qDebug()<<"y[0] = "<<this->y[0]<<"; y[1] = "<<this->y[1]<<"; y[2] = "<<this->y[2];

    }

    //qDebug()<<"g_x[0] = "<<this->g_x[0]<<"; g_x[1] = "<<this->g_x[1];
    //qDebug()<<"g_y[0] = "<<this->g_y[0]<<"; g_y[1] = "<<this->g_y[1];
...
void WingedEdge::merge(WingedEdge S_l, WingedEdge S_r)
{
    ...
    vector<int> vertex_w_l = S_l.get_w(), vertex_w_r = S_r.get_w();
    ...
    //next_Px is Pz corresponding to left_edge; next_Py is Pz corresponding to right_edge
    int next_Px,next_Py;
    //recording previous Px and Py
        ...
        if(have_inter_l && have_inter_r &&fabs(inter_l_x-inter_r_x) < 1e-8 && fabs(inter_l_y-inter_r_y) < 1e-8){
            ...
            if(!top){
                ...
                *this->ccw_successor.end() = this->num_edges - 1;
                ...
                this->end_vertex.push_back(vertex_to_move);
            ...
            if(cross_product(prev_x,prev_y,inter_x,inter_y,vertex_x_l[start_vertex_l[left_edge]],vertex_y_l[start_vertex_l[left_edge]]) > 0){
                ...
                qDebug()<<"merge: Both start_vertex and end_vertex of left_ray are not on the left side of BS";
                qDebug()<<"left_edge = "<<left_edge;
                qDebug()<<"inf_rays_Sl[Px] = "<<inf_rays_Sl[Px];
                qDebug()<<"this->start_vertex[left_edge] = "<<this->start_vertex[left_edge];
                qDebug()<<"this->x[this->start_vertex[left_edge]] = "<<this->x[this->start_vertex[left_edge]];
                qDebug()<<"this->y[this->start_vertex[left_edge]] = "<<this->y[this->start_vertex[left_edge]];
                qDebug()<<"this->x[this->end_vertex[left_edge]] = "<<this->x[this->end_vertex[left_edge]];
                qDebug()<<"this->y[this->end_vertex[left_edge]] = "<<this->y[this->end_vertex[left_edge]];
                //exit(-1);
            }
            /******************************************************************/
            ...
            */
            ...
            if(cross_product(prev_x,prev_y,inter_x,inter_y,vertex_x_r[start_vertex_r[right_edge]],vertex_y_r[start_vertex_r[right_edge]]) < 0){
                ...
                if(this->end_vertex.back() == (end_vertex_r[right_edge] + num_v_l)){
                    ...
                    vertex_to_move = end_vertex_r[right_edge] + num_v_l;
                }
            }
            else{
...
/*******************************       End of while      ************************************/
    ...
    else if(S_r.getNumPolygons() ==1){
        //When right voronoi has only 1 point
        this->start_vertex[edge_to_be_config_r] = vertex_to_be_config;
    }
    else{
        //For debug
